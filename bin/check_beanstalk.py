...
# encoding: utf-8
'''
check_beanstalk -- shortdesc

check_beanstalk is a Nagios plugin that uses SNMP to query a custom
back end that returns results from a beanstalk daemon.

It defines classes_and_methods

@author:     Erinn Looney-Triggs
        
@copyright:  2013 Erinn Looney-Triggs. All rights reserved.
        
@license:    AGPLv3

@contact:    erinn.looneytriggs@gmail.com
...
def presentResults(results):
    ...
    for item in ([3, 9, 10] + range(13, 15)):
...
def setAlarms(results, args):
    ...
    if int(results[3][-1]) >= ready_warning:
        exit_state['warning'] += 1
        if int(results[3][-1]) >= ready_critical:
            exit_state['critical'] += 1
    
    if int(results[9][-1]) <= workers_warning:
        exit_state['warning'] += 1
        if int(results[9][-1]) <= workers_critical:
            exit_state['critical'] += 1
...
if __name__ == "__main__":
    ...
    Created by Erinn Looney-Triggs on {1}.
    Copyright 2013 Erinn Looney-Triggs. All rights reserved.
    ...
    parser.add_argument('--ready-critical', type=int, 
                        dest='ready_critical',
                        help=('Critical threshold for the number of current '
                              'jobs ready'))
    parser.add_argument('--ready-warning', type=int,
                        dest='ready_warning',
                        help=('Warning threshold for the number of current '
                              'jobs ready'))
    ...
    parser.add_argument('--workers-critical', type=int, 
                        dest='workers_critical',
                        help=('Critical threshold for the number of workers '
                              'running'))
    parser.add_argument('--workers-warning', type=int,
                        dest='workers_warning',
                        help=('Warning threshold for the number of workers '
                              'running'))
    ...
    results = queryBeanstalk(session)
